...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='D:\\Facial Expression Recognition\\表情识别数据集\\Oulu_splitted', help='Raf-DB dataset path.')
    parser.add_argument('--checkpoint', type=str, default='new2_models/Oulu',
...
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase, transform = None, basic_aug = False):
        ...
        self.file_paths = []
        self.label = list()
        class_names = os.listdir(os.path.join(self.raf_path, 'train'))
        sorted(class_names)
        ...
        with open(os.path.join(args.checkpoint, 'cls2id.json'), 'w') as fh:
            json.dump(cls2id, fh)
        if self.phase == 'train':
            self.file_paths = get_images(os.path.join(self.raf_path, 'train'))
        else:
            self.file_paths = get_images(os.path.join(self.raf_path, 'val'))
