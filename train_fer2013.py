...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='D:\\Facial Expression Recognition\\表情识别数据集\\CK+48_fer2013',
                        help='Raf-DB dataset path.')
    # parser.add_argument('--raf_path', type=str, default='D:\\Facial Expression Recognition\\表情识别数据集\\Oulu_CASIA_NIR_VIS\\NI\\Strong',
    #                                        help ='Raf-DB dataset path.')
    parser.add_argument('--checkpoint', type=str, default='models/fer2013+',
...
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase, transform=None, basic_aug=False):
        ...
        self.file_paths = []
        self.label = list()
        class_names = os.listdir(os.path.join(self.raf_path, 'Training'))
        sorted(class_names)
        ...
        with open(os.path.join(args.checkpoint, 'cls2id.json'), 'w') as fh:
            json.dump(cls2id, fh)
        if self.phase == 'train':
            self.file_paths = get_images(os.path.join(self.raf_path, 'Training'))
        else:
            self.file_paths = get_images(os.path.join(self.raf_path, 'PublicTest'))
